...
impl SockAddr {
    ...
    pub fn is_unnamed(&self) -> bool {
        self.as_sockaddr_un()
            .map(|storage| {
                self.len() == offset_of_path(storage) as socklen_t
    ...
    pub fn as_pathname(&self) -> Option<&Path> {
        self.as_sockaddr_un().and_then(|storage| {
            (self.len() > offset_of_path(storage) as socklen_t && storage.sun_path[0] != 0).then(
                || {
    ...
    pub fn as_abstract_namespace(&self) -> Option<&[u8]> {
        ...
        {
            self.as_sockaddr_un().and_then(|storage| {
                (self.len() > offset_of_path(storage) as socklen_t && storage.sun_path[0] == 0)
